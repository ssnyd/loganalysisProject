 ...
 */
public class ApplyEStatImpl implements IApplyEStatDAO {
    @Override
    public void updataBatch(List<applyStat> applyStats, Connection conn,Integer num) {
        ...
        String selectSQL = "SELECT count(*) "
                + "FROM rp_app_enterprise_action_num "
                + "WHERE created=? "
                + "AND rpAppId=? "
                + "AND instanceId=? "
                + "AND action=? ";
        ...
        // 执行插入操作 4个参数
        if (num == 0){
            insertSQL = "INSERT INTO rp_app_enterprise_action_num(created,rpAppId,action,instanceId,num) VALUES(?,?,?,?,?)";
            updateSQL = "UPDATE rp_app_enterprise_action_num set "
                + "num=? "
                + "WHERE created=? "
                + "AND rpAppId=? "
                + "AND instanceId=? "
            ...
            insertSQL = "INSERT INTO rp_app_enterprise_action_num(created,rpAppId,action,instanceId,memberNum) VALUES(?,?,?,?,?)";
            updateSQL = "UPDATE rp_app_enterprise_action_num set "
                + "memberNum=? "
                + "WHERE created=? "
                + "AND rpAppId=? "
