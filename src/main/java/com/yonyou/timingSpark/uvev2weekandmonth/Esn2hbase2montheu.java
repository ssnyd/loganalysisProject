 ...
 */
public class Esn2hbase2montheu {
    public static void main(String[] args) {
        SparkConf sconf = new SparkConf()
                .setAppName("Esn2hbase2montheu")
            ...
            uvmonthrdd.foreachPartition(new VoidFunction<Iterator<String>>() {
                ...
                public void call(Iterator<String> iterator2) throws Exception {
                    ...
                    while (iterator2.hasNext()) {
                        ...
                        if (lines2.length == 3) {
                            Put put2 = new Put((String.valueOf(lines2[0] + ":" + lines2[1]+":"+lines2[2]).getBytes()));
                            put2.addColumn(Bytes.toBytes("eu"), Bytes.toBytes("log"), Bytes.toBytes(DateUtils.getTimestamp(lines2[0]) + ":" + lines2[1]+":"+lines2[2]));
                            puts2.add(put2);
                        }
                    }
                    if (puts2.size() > 0) {
                        HTable hTable2 = HbaseConnectionFactory.gethTable("esn_month_eu", "eu");
                        hTable2.put(puts2);
                        hTable2.flushCommits();
                        System.out.println("hbase ==> esn_month_eu " + puts2.size());
                        puts2.clear();
                        if (hTable2 != null) {
                            hTable2.close();
                        }
